...
public class SchematicsDialogOverride extends SchematicsDialog {
    private SchematicInfoDialog info = new SchematicInfoDialog();
    private Schematic firstSchematic;
    private String search = "";
    private TextField searchField;

    public SchematicsDialogOverride() {
        super();
        getListeners().pop(); // shown
        getListeners().pop(); // onResize

        // Add the one patched one
        shown(this::setup);
        onResize(this::setup);
    }
        ...
        cont.pane(t -> {
            ...
            rebuildPane[0] = () -> {
                ...
                for(Schematic s : schematics.all()) {
                    if(!search.isEmpty() && !s.name().toLowerCase().replaceAll(regex, " ").contains(searchString))
                        continue;
                    ...
                    sel[0] = t.button(b -> {
                        ...
                        b.table(buttons -> {
                            ...
                            buttons.button(Icon.pencil, style, () -> new Dialog("@schematic.rename") {{
                                ...
                                TextField descripionField = cont.area(s.description(), Styles.areaField, t -> {
                                }).size(400f, 140f).addInputDialog().get();
                    ...
                    sel[0].clicked(KeyCode.mouseRight, () -> {
                        GlobalImageUnban.useSchematic(s);
                        hide();
                    });
            ...
            cont.table(r -> {
                ...
                for(ItemStack s : arr) {
                    ...
                    r.label(() -> {
                        ...
                        if(core == null || state.rules.infiniteResources || core.items.has(s.item, s.amount))
